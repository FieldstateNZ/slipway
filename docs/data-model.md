...
| `task`            | A node in the dependency graph: `project`, `pr` (priority order), `owner` (`you`/`them`), `kind` (`action`/`decision`/`provide`), `effort_min`, display copy (`title`, `short`, `sub`, `frees`), `in_progress`, `done`, `done_at`.                      |
